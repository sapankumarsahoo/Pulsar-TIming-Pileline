...
#set output = `pwd`
set output="/home/bhaswati/Timing_study/analysis"
set par_location="/home/bhaswati/Timing_study/"

echo "location of the script timing file: ${code}/timing_GWB_GPTOOL_FREQ_OFFSET.csh"
echo "location of the script header file: ${code}/gmrt_hdr.timing_GWB_GPTOOL_FREQ_OFFSET.csh"
echo "Output will be save on themlocation: ${output}"
echo "Location of par file: "${par_location}/${scan}"*par"
...
$code/gmrt_hdr.timing_GWB_GPTOOL_FREQ_OFFSET.csh $date $freq $rawdir $dedisp_type $mode $nchan $raw_id
...
while ($i <= $numfile)
	...
	if ("$dedisp_type" == "I")then
		if ("$raw_id" == 0)then
			...
			#GPTool
	                cp /home/bhaswati/soft/scripts/header_files/gptool.${freq}MHz_${bandwidth}MHzBW_${nchan}channel.in gptool.in
        	        /home/ssahoo/soft/gptool_ver4.4.4/gptool -f ${rawdir}/${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw -o . -m 32 -nodedisp  -t 4
		        ...
		        filterbank ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.gmrt_dat > ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil
	                ...
	                prepfold -timing *par ${scan}*fil -n 128 -nsub 128 -noxwin -o ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.new_freq.gptool -n 256
			...
			set tres=`cat ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.${raw_id}.gmrt_hdr | grep "Sampling Time" | awk -F: '{print $2}'`
       	 		...
       	 		#GPTool
			cp /home/bhaswati/soft/scripts/header_files/gptool.${freq}MHz_${bandwidth}MHzBW_${nchan}channel.in gptool.in
			/home/ssahoo/soft/gptool_ver4.4.4/gptool -f ${rawdir}/${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.${raw_id} -o . -m 32 -nodedisp  -t 4
			...
			prepfold -timing *par ${scan}*fil -noxwin -o ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.${raw_id}.new_freq.gptool -n 128
	       		echo "Folding is complete for $scan" >> Time.log
		...
		#GPTool
		cp /home/bhaswati/soft/scripts/header_files/gptool.${freq}MHz_${bandwidth}MHzBW_${nchan}channel.in gptool.in
                /home/ssahoo/soft/gptool_ver4.4.4/gptool -f ${rawdir}/${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.dat -o . -m 32 -nodedisp  -t 4
		...
		filterbank ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.gmrt_dat > ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.fil
                ...
                prepfold -n 32 -nsub 128 -timing ${scan}*par ${scan}*fil -noxwin -o ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.new_freq.gptool
