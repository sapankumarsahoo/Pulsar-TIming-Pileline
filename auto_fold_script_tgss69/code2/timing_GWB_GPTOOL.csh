...
set code=/data/scripts/code2 
set output = `pwd`

#set output=/data/timing/
...
$code/gmrt_hdr.timing_GWB_GPTOOL.csh $date $freq $rawdir $dedisp_type $mode $nchan $raw_id
...
while ($i <= $numfile)
	...
	cd $scan
	rm -rf *par *pfd*
	cp /data/scripts/code2/parfile/${scan}*par . 
	...
	if ("$dedisp_type" == "I")then
		if ("$raw_id" == 0)then
			...
			#GPTool
	                cp /data/scripts/header_files/gptool.${freq}MHz_${bandwidth}MHzBW_${nchan}channel.in gptool.in
		        ...
		        filterbank ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.gmrt_dat > ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil
	                ...
	                prepfold -timing *par ${scan}*fil -noxwin -o ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.gptool.29dec_550
			...
			prepfold -timing *par ${scan}*fil -noxwin -o ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.${raw_id}.gptool.29dec_550
	       		echo "Folding is complete for $scan" >> Time.log
		...
		filterbank ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.gmrt_dat > ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.fil
                ...
                prepfold -n 128 -nsub 128 -timing ${scan}*par ${scan}*fil -noxwin -o ${scan}_${freq}_${bandwidth}_${nchan}_${int_factor}.${date}.raw${raw_id}.gptool.29dec_550
                echo "Folding is complete for $scan" >> Time.log
