...
if($#argv<12) then
    echo " "
    echo "Usage: timing.csh frequency_(MHz) [322 or 607] full_data_dir [/data/data/31_036_10dec2k16.2048.322.GHRSS] DM [38.000] Period [4.84] bin[32] backend[GSB/GHRSS_2K/GHRSS_1K/GWB] mode [ia/pa] scratchdir [/data/analysis/analysis/] zmax [0 or 200] numharm [8 or 16] dspsr [0 for both prepso-dspsr, 1 for only dspsr] raw_id[0-5]"
    echo "Exiting..... "
    exit
  endif
...
$code/gmrt_hdr.timing_GWB_anki.csh $date $freq $rawdir $dedisp_type $mode $nchan $raw_id
	...
	if ("$dedisp_type" == "I")then
		if ("$raw_id" == 0)then
			...
			echo "No candidate matched"
				...
				echo "Period, Pd and Pdd are $topo_ps $topo_pd $topo_pdd"
			...
			accelsearch -zmax $zmax -wmax 600 -numharm $numharm ${scan}_search*.fft

                        #Candidate selection
			set cand1 = `cat ${scan}_search*_ACCEL_${zmax}_JERK_600  | awk '{print $1 " " $6}' | grep " ${period_1}\.${period_2}" | awk '{print $1}'`
			echo "candidate number is $cand1"

			set length1 = `echo "$cand1" | wc -w`

			echo "$length1 candidate(s) found"
			...
			if ( "$cand1" == "" )then
				...
				cat ${scan}_search*_ACCEL_${zmax}_JERK_600  | awk '{print $1 " " $6}' | grep " ${period_1}\.${period_2}" | awk '{print $1 " " $2}' >> ${date}_possible_candidates
                                
				set cand1=`cat ${date}_possible_candidates | head -$i | tail -1 | awk '{print $1}'`

				echo "Candidate number is $cand1"

				prepfold -accelcand $cand1 -accelfile ${scan}_search*_ACCEL_${zmax}_JERK_600.cand -dm $dm -nodmsearch -nosearch -noxwin ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil -o ${scan}_search_bary.${date}.${freq}
				set topo_p1=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep P_topo | awk -F= '{print $2}' | awk -F+ '{print $1}'`
				set topo_pd1=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep "P'_topo" | awk -F= '{print $2}' | awk -F+ '{print $1}'`
				set topo_pdd1=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep "P''_topo" | awk -F= '{print $2}' | awk -F+ '{print $1}'`
				MATH1 topo_ps1 = $topo_p1/1000.0
				echo "Period, Pd and Pdd are $topo_ps1 $topo_pd1 $topo_pdd1"
				prepfold -dm $dm -nodmsearch -n $bin -p $topo_ps1 -pd $topo_pd1 -pdd $topo_pdd1 -nosearch -topo -noxwin ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil -o ${scan}_search_topo.${date}.${freq}_JERK
		...
		accelsearch -zmax $zmax -wmax 600 -numharm $numharm ${scan}_search*.fft
                ...
                #Candidate selection
                set cand = `cat ${scan}_search*_ACCEL_${zmax}_JERK_600  | awk '{print $1 " " $6}' | grep " ${period_1}\.${period_2}" | awk '{print $1}'`
                echo "candidate number is $cand"
			...
			prepfold -accelcand $cand -accelfile ${scan}_search*_ACCEL_${zmax}_JERK_600.cand -dm $dm -nodmsearch -nosearch -noxwin ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil -o ${scan}_search_bary.${date}.${freq}
			set topo_p=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep P_topo | awk -F= '{print $2}' | awk -F+ '{print $1}'`
			set topo_pd=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep "P'_topo" | awk -F= '{print $2}' | awk -F+ '{print $1}'`
                        set topo_pdd=`cat ${scan}_search_bary.${date}.${freq}_JERK*bestprof | grep "P''_topo" | awk -F= '{print $2}' | awk -F+ '{print $1}'`
                        MATH1 topo_ps = $topo_p/1000.0
                        echo "Period, Pd and Pdd are $topo_ps $topo_p $topo_pd $topo_pdd"
                        prepfold -dm $dm -nodmsearch -n $bin -p $topo_ps -pd $topo_pd -pdd $topo_pdd -nosearch -topo -noxwin ${scan}_${mode}_${freq}_${bandwidth}_${nchan}_${int_factor}_1_8_${date}.raw.fil -o ${scan}_search_topo.${date}.${freq}_JERK
